...
2026-08-09 這關又抓到三種不同的死法，全都是前三關過關的模型：

- `openai/gpt-oss-120b`：每一項組成都對（+18,000 / −3,000 / 合計 +15,000），
  但自己算的那個**總資產寫成 303,000**（正解 353,000）。**它只算錯它自己算的那一個數字** ——
  這正是為什麼 `Facts.gs` 要把數字先算好餵進去，而不是讓模型加總。
- `minimaxai/minimax-m3`：推理 1258 token 吃光預算，`finish_reason=length`，正文 `null`。
- `nvidia/nemotron-3-super-120b-a12b`：**把整段英文推理當成正文吐出來**，
  裡面還在自問自答「Profit = 353000 − 218000 = 135000? Wait check」，最後沒有產出答案。

⚠️ 順帶驗證：`/no_think` 對 **Nemotron 3** 無效（reasoning 478 → 1180，不降反升），
但它對上一代 `llama-3.3-nemotron-super-49b` 是有效的（493 → 0）。
**同廠牌上一代有效的寫法，不能假設這一代通用。**
