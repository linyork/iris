...
### D3 — 工具契約是字串，錯誤與空資料無法區分　｜高｜階段 0（**大部分已修**）
...
| 資料 | 實際時效 | 模型知道嗎 |
|---|---|---|
| `指標` 的總資產 | **上一次 `Position.rebuild()` 當下**（13:00 排程、記帳、`/refresh`）| ⚠️ `getDashboard` 會印出「最後重算」那一列（它把 `指標` 整張逐列印出來），但**只有走 getDashboard 才看得到**，而且它夾在十幾個指標中間，沒有標明那就是這些數字的時點 |
| `每日快照` 的歷史序列 | 每天 18:00 寫一次 | ❌ `getHistory` 只印日期與總資產。`totalSeries` 明明帶了 `狀態`（休市／資料未更新／報價異常），排版時丟掉了 —— 平掉的那一段是放假還是抓取失敗，模型分不出來 |
| `持倉` 的市價／市值 | GOOGLEFINANCE 活公式，更新時機不定 | ❌ `getHoldings` 完全沒有時點 |
| TWSE 即時報價（當日漲跌）| 延遲約 20 分鐘，收盤後沒有當日價 | ✅ **已修（2026-08-08）**：沒有當日成交價時回 `null` 而不是 0，輸出明講取不到。見 CLAUDE.md〈拿不到就說拿不到〉 |
| `_fillMissingPrices` 補的價 | **死值**，不會自己更新到下次 rebuild | ⚠️ 只知道 `priceMissing`（而且那是「抓不到」的意思），不知道現在看到的價是備援補上去的 |

⚠️ 初次校準時我把第一列寫成「完全看不到」，那是**錯的** —— `getDashboard` 逐列印 `指標`，
「最後重算」本來就在裡面。真正的缺口比較窄：**時點沒有跟著數字走**，而是散落在別的工具的
輸出裡，問持倉的時候就看不到了。
