...
var Eval = (() => {
  ...
  ev._askInner = (question, id, toolOutput) => {
    var event = {
      // ⚠️ userId 要「每題不同」**而且「每次執行不同」**。
      //
      //    只做前者是不夠的 —— 2026-08-09 第二輪基準線就踩到了：同一題第二次跑，
      //    userId 仍是 `EVAL:Q04`，於是它讀到第一輪自己的問答，直接從對話歷史抄
      //    300,000 股而沒有再叫 getHoldings。判定看不到工具回傳，就報「數字沒出處」。
      //
      //    更糟的是那正是系統提示詞明令禁止的行為（不准引用對話歷史裡的數字），
      //    而評估本來該抓它 —— 結果評估自己製造了那個環境。加上時間戳，每次都是新的人。
      //
