 ...
 *
 * 拆成兩半：
 *   判定 Eval.CHECKS —— 純函式 (reply, ctx) => {ok, why}，不打 LLM、不讀表，可測（T37）
 *   執行 Eval.runBatch —— 真的跑一次 ChatBot.reply，需要 LLM
 *
 * ⚠️ 判定的是性質不是字串。期望值不可寫「應該回答 142萬」——隔天就過期，
 *    而且變成對答案而不是對行為。要問的是有沒有講時點、數字有沒有出處、
 *    是非題有沒有先答是否。
 * ⚠️ 一次只跑幾題（GAS 6 分鐘上限，一題可能跑完整個 ReAct 迴圈）。
 *    runBatch 挑最舊的幾題並寫回結果，重複執行直到整組跑完。
 */
var Eval = (() => {
  var ev = {};
  ...
  // 這次執行的識別碼。GAS 每次執行都重新載入全部 .gs，所以它每次執行都是新的 ——
  // 跟 `Utils.EXEC_START` 同一個道理。用途見 `_askInner` 裡 userId 的註解。
  var RUN_STAMP = String(Date.now());
  ...
  /**
   * 回覆裡的數字能不能在脈絡（事實區塊 + 工具回傳）裡找到。
   * 最有價值也最容易誤判的一條，放寬的地方：
   * - 「142萬」是人設要求的寫法，所以 n 與 n×10000 都算命中
   * - 一萬以下不檢查（年份、百分比、股數、列號、日期都落在這裡）
   * - 只比整數部分，允許四捨五入
   */
  ...
  ev.CHECKS = {
    ...
    yesNoFirst: (reply) => {
      ...
      // 判的是「第一行裡有沒有答案」，不是「答案有沒有落在第一個字」。
      // 人設要防的是把一個「對」寫成三段，而「以你的設定來說，不高。」
      // 這種一句話就給完答案的寫法沒有那個毛病 —— 卡在字首會誤殺它。
      // 純鋪陳（整行沒有任何是否詞）仍然擋下。
      var ANSWER = /(不算|不高|不用|不需要|不建議|不行|不會|沒有|還不到|還沒|是的|對的|可以|建議|應該|算是|還好|^是|^否|^對|^不|^有|^會)/;
      return ANSWER.test(head)
        ? { ok: true, why: '' }
        : { ok: false, why: '第一行沒有給出是／否：' + head.slice(0, 34) };
    },
  ...
  ev._askInner = (question, id, toolOutput) => {
    var event = {
      // ⚠️ userId 要每題不同**而且每次執行不同**。只做前者的話，同一題第二次跑
      //    會讀到第一輪自己的問答，直接從對話歷史抄數字而不再呼叫工具 ——
      //    那正是提示詞禁止、而評估該抓的行為，等於評估自己製造了那個環境。
      // ⚠️ platform 不可設成 'TELEGRAM'：那會讓 indicateTyping 去打 Bot API，
      //    把「正在輸入」推給主人。評估是背景作業。
      platform: 'EVAL', replyToken: 'eval', sourceId: 'eval',
      source: { type: 'user', userId: 'EVAL:' + (id || 'X') + ':' + RUN_STAMP },
