 ...
 *
 * `Logger.ai` 有記延遲、`ChatBot` 有記輪數與耗時，但沒有人加總，所以回答不了
 * 「平均幾輪收斂」「哪個工具最常被叫」「備援接手幾次」「假宣稱攔截觸發過嗎」。
 * 更糟的是 `consolelog` 每 10 天就被 `dailyCleanUp` 清掉 —— 那些資料一直在被丟。
 *
 * ⚠️ **一定要在清 consolelog 之前跑。** `dailyCleanUp` 的順序是有意義的：
 *    聚合排在清理前面，否則被清掉的那幾天就永遠算不回來了。目前保留 10 天、
 *    每天聚合最近 3 天，中間有很大的餘裕，但順序不能倒過來。
 *
 * ⚠️ **同一天重跑會覆蓋，不會疊加。** 排程失敗補跑、手動執行都不該產生兩列。
 *    也因為這樣，每次回頭算 3 天而不是只算昨天 —— 漏掉的那天下次會自己補上。
 */
var Metrics = (() => {
  var m = {};
   ...
   * consolelog 那一格的日期部分。
   *
   * ⚠️ **字串不要再解析成 Date。** `GoogleSheet.setLog` 寫進去的已經是
   * `Utilities.formatDate(..., 'GMT+8', 'yyyy/MM/dd HH:mm:ss')` —— 一個已經在
   * GMT+8 的字串。再 `new Date(...)` 一次會用執行環境的時區去理解它，然後
   * 又用 GMT+8 格式化回來，兩次時區換算不對消的話整天會偏移一格，
   * 而偏移的症狀是「昨天的資料算不到」，不會報錯。直接切前 10 個字最穩。
   *
   * Sheets 有時會把那格自動轉成真的日期物件，所以兩種都要接。
   */
  var _dayOf = (cell) => {
