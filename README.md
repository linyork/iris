...
- `Tools.isWrite(name)` 標出十個會真的動到試算表的工具，`ChatBot.reply` 記下這一輪有沒有執行過
...
> ⚠️ 新增寫入工具時要一起加進 `Tools.gs` 的 `WRITE_TOOLS` —— 這是繼 `definitions` 與
> `execute()` switch 之後的第三個必須同步的地方。漏了不會報錯，只是那個工具的假宣稱不再被攔。
