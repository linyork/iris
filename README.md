...
> | `Tools.gs` 的 12 個工具 | `@tool` decorator / `StructuredTool` |
...
| `DataSync.gs` | 每日 18:00 寫入 `@所有股票紀錄` 快照；欄位以標題文字定位並自動補欄（見「快照欄位契約」） |
...
| `@所有股票紀錄` | 每日資產快照（`setData` 於 18:00 寫入）<br>欄位順序：`日期｜總價值｜各持股股價…｜股票總價值｜各現金帳戶…｜狀態` |
| `@股利` | 股利收入明細（日期、代號、金額） |
| `@固定` | 固定資產（黃金重量），由 `Snapshot._gold()` 讀取 |

### ⚠️ 快照欄位契約

`@所有股票紀錄` 的欄位一律**以標題文字定位**，不可用持股數量推算位置。

舊版是這樣算的：`sumStartCol = 2 + numStocks + 1`。但標題列是手動維護的，所以
**只要「所有股票」多一檔，資料就整排右移而標題不動** —— 而且不會報錯：`總價值`
的 SUM 範圍會偏掉，`GoogleSheet.getHistory()` 拿標題配值餵給 LLM 時每一列都是錯的
標籤。將近 950 天的歷史資料，距離「一檔新 ETF 掛牌就全毀」只差一步。

現在 `DataSync.syncHeader()` 會在每次寫入前先對齊標題列：

- **只插入、永不刪除** —— 賣掉的標的其歷史欄位要留著
- 新持股插在 `股票總價值` 之前；新現金帳戶插在 `狀態` 之前
- 用 `insertColumnBefore()`，讓 Sheets 自動修正既有列的 `總價值` 公式範圍
- 持股欄以**名稱**為鍵（既有 950 列就是這樣存的）。在 `所有股票!B` 改名會被視為新欄位，
  要改名的話快照標題列也要一起改
- 標題列找不到 `股票總價值` 這個錨點時直接**丟例外**，寧可不寫也不寫錯位置

另外兩道保險：

- **同一天重跑會覆寫當日那列**，不會多長一列（手動重跑、Trigger 重試都安全）
- **股價全數無效時放棄寫入**（`#N/A`、`Loading…`、非正數 —— GOOGLEFINANCE 真的會掛，
  看 `consolelog`）。少一天可以補，一整列 0 會污染所有下游百分比。只有部分無效時，
  該欄留空並標記 `報價異常`

`狀態` 欄記錄這列為什麼看起來沒動：`交易日` / `休市`（週末）/ `資料未更新`（股價與前一列
完全相同 —— 國定假日或抓取失敗）/ `報價異常`。專案裡沒有台股行事曆，所以這是推論不是
權威判定。算波動度或日變化統計前先用它過濾 —— 在加入這欄之前的資料裡，有 19 列是週末
照抄前一日的。

**在「所有股票」新增標的之前，先在 GAS 編輯器執行 `verifyRecordSheet()`**，它會回報將要
插入哪些欄位、插在哪裡，不寫入任何資料。`dryRunSetData()` 則可預覽今天會寫入的整列內容。

---

## AI 工具集

`Tools.gs` 共定義 12 個工具，呼叫者為 LLM。
...
| 每日 18:00 | `setData` | 寫入當日資產快照至 `@所有股票紀錄`（自動對齊標題列、同日覆寫） |
